...
sys.path.append('C:\Program Files\MySQL')
...
localConfig = {
    ...
    # 游标类型，它返回的结果集是字典形式的 执行一个查询并且获取结果时，每一行数据都会以字典的形式返回，其中键是列名，值是对应的数据。
}  # 数据库配置连接
...
class Staff:
    ...
    def __init__(self, config=localConfig):
        ...
        # SQL查询，它调用MySQL的 VERSION() 函数来获取当前MySQL服务器的版本信息
        data = self.cursor.fetchone()
        # 从查询结果中取出第一行数据
        print(f"Database version :{data['VERSION()']} ")
        # data 是一个字典类型的表，'VERSION()'是列名
        # 能打印说明连上了
        self.password = None
        self.srole = None
        self.sid = None
    ...
    def userLogin(self, username, password):
        ...
        try:
            ...
            # 所有行
            for row in data:
                if row['sid'] == username and row['spassword'] == password:
    ...
    def addStaff(self, sid, sname, ssex, spassword, srole, sidcard, sphone):

        try:
            self.cursor.execute("insert into staff values(%s,%s,%s,%s,%s,%s,%s)",
...
class Room:
    ...
    def showRoom(self, rtype, rstate, rstorey, rstarttime, rendtime, price_bottom, price_up):
        """根据条件检索房间"""
        """"
        如果 rstate等于0，表示查询末被占用的房间。函数执行SQL查询，选择满足类型、楼晨和价格范围条件的所有房间，并将结果存储在 dat01中，然后返回这些数解.。如果 rstate 等于1，表示查询已被占用或预订的房间。这需要更复杂的逻辑。查询已被占用或预订的房间:
        首先，执行 SQL查词获取满足基本条件(类型、楼层、价格范围)的房间ID 列表，并存储在 data 中。
        然后，对于每个房间1D，函教检查是否有客户入住或团队预订的记录，这些记录与给定的时间范围冲突，这是通过在 checkin cient、checkin (team,boking,cient 和 boking temn 表中热行 S
        检查房间可用性:
        对于每个房间D，如果它在所有四个表中都没有冲实的记录(即，data1、dat62、dta3 和 dat34 都为空)，则认为液房间在给定时间范围内可用，并将房间 1D 添加到 kt dath 列表中。
        返回可用房间的详细信息
        最后，对于istdata 中的每个房间ID，执行 SQL查询以获取房间的详细信息，并将这些信息累加到ret列表中。然后返回 ret 列表。
        """
        ...
        if rstate == 0:
            ...
            for i in range(len(data)):
                crid = data[i]['rid']
                self.cursor.execute(
                    "select * from hotelorder as A where (A.rid=%s) and (A.end_time>%s and A.start_time<%s "
    ...
    def singleCheckinDB(self, cname, cid, cphone, cage, csex, crid, cendtime, remark):
        ...
        totalPrice =1000
        self.cursor.execute("insert into client(cname,cid,cphone,cage,csex,leader) "
                            "values(%s,%s,%s,%s,%s,%s)", (cname, cid, cphone, cage, csex, cname))
        self.cursor.execute("insert into hotelorder values(%s,%s,%s,%s,%s,%s,%s)",
    ...
    def reserveToCheckinT(self, tid, rrid):
        ...
        for rid in re.split('[,， ]', rrid):
            ...
            print(data)
            if data == ():
                QMessageBox().information(None, "提示", "%s房间没有对应预约或者预约入住时间未到！" % rid,
                                          QMessageBox.Yes)
                return False
            # 再从预约表中获取相关信息
            endtime = data[0]['end_time']
    ...
    def reserveCDB(self, cname, cid, cphone, cage, csex, crid, cstarttime, cendtime, cremark):
        """个人预约"""
        starttime = datetime.date.today()
        self.cursor.execute("select * from hotelorder as A where (A.rid=%s) and (A.end_time>%s and A.start_time<%s "
    ...
    def reserveTDB(self, tname, tid, tphone, ttrid, tstarttime, tendtime, tremark):
        ...
        for trid in re.split(',|，| ', ttrid):
            ...
            try:
                self.cursor.execute(
                    "insert into booking_team(tid,rid,start_time,end_time,remark) values(%s,%s,%s,%s,%s)",
    ...
    def checkoutDB(self, flag, id, rid, payType, remark):
        ...
        try:
            if flag == '个人':
                self.cursor.execute("select * from hotelorder where id=%s and rid=%s", (id, rid))
                data = self.cursor.fetchall()
                if data == ():
                    QMessageBox().information(None, "提示", "没有相关入住信息！", QMessageBox.Yes)
                    return False
                else:
                    # 外键约束下要把compare里面的也删了
                    self.cursor.execute("delete from compare where id=%s", (id,))
                    self.cursor.execute("delete from hotelorder where id=%s and rid=%s", (id, rid))
                    self.db.commit()
...
class Chart:
    ...
    def getRevenue(self):
        ...
        for i in range(7):
            ...
            list_revenue= [2000, 3000, 1000, 1500, 500, 2000, 3500]
        print(list_revenue)
        print(list_date)
        list_date.reverse()
    ...
    def getOccupy(self):
        ...
        for i in range(7):
            ...
            list_occupy = [0.333, 0.4, 0.2, 0.2, 0.1, 0.42, 0.54]
        print(list_occupy)
        list_date.reverse()
    ...
    def getClientStatics(self):
        ...
        """
        list_clientStatics = []
        self.cursor.execute("select * from hotelorder")
...
class Ui_LoginWindow(object):
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("浙工商五星级大酒店")
        ...
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("hotel.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        ...
        MainWindow.setStyleSheet("\n"
                                 ...
                                 "}\n"
                                 "#centralwidget{\n"
                                 "border-image:url(background.jpg) strectch；\n"
                                 "}\n"
                                 "\n"
                                 "#toolButton{\n"
                                 "background:red;\n"
        ...
        icon1 = QtGui.QIcon()
        icon1.addPixmap(QtGui.QPixmap("ui/hui.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "浙工商五星级大酒店"))
...
class Ui_HomeWindow(object):
    def setupUi(self, MainWindow):
        ...
        icon2 = QtGui.QIcon()
        icon2.addPixmap(QtGui.QPixmap("coffee.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        ...
        icon3 = QtGui.QIcon()
        icon3.addPixmap(QtGui.QPixmap("staff.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        ...
        icon4 = QtGui.QIcon()
        icon4.addPixmap(QtGui.QPixmap("hotel.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label.setText(_translate("MainWindow", "浙工商五星级大酒店"))
...
class LoginPage(QMainWindow, Ui_LoginWindow):  # 我将其设置为 main control 页面跳转
    ...
    def display(self):
        ...
        staff = _initStaff()
        role = staff.userLogin(username, password)
        # 将获取到的密码（以及用户名）传递给 staff.userLogin(username, password) 方法进行验证。
...
class HomePage(QMainWindow, Ui_HomeWindow):
    def __init__(self, parent=None):
        ...
        self.setupUi(self)
        self.staff = get_staff()
        print(self.staff.sname)
        if not self.staff:
            QMessageBox().critical(self, "错误", "无法初始化员工信息")
            self.close()
            return
        self.initializeUI()
    ...
    def initializeUI(self):
        self.welcome.setText(
            self.staff.sname + ',你好。你的权限为：' + str(self.staff.srole) + '。今天是' + time.strftime("%Y-%m-%d",
                                                                                                  time.localtime()))
...
class ChartOp(QMainWindow, Ui_ReportWindow):
    def __init__(self, parent=None):
        ...
        self.role.setText('权限：' + str(self.staff.srole))
    ...
    def plotStaff(self):
        ...
        cols = ['r', 'm']
        F1.axes.pie(component, labels=content, startangle=90, shadow=True, explode=(0, 0.1), colors=cols,
                    autopct='%1.1f%%')
...
class RoomOp(QMainWindow, Ui_RoomWindow):
    ...
    def showRoom(self, rid, rtype, rstorey, rprice, rdesc, rpic, endtime, i, j):
        ...
        self.flayout.addWidget(
            QLabel("描述:" + rdesc + " 价格:" + rprice, self, styleSheet="color: #990066;", openExternalLinks=True))
        pb = QPushButton(self)
...
class StaffOP(QMainWindow, Ui_StaffWindow):
    def __init__(self, parent=None):
        ...
        self.srole.setText(str(self.staff.srole))
    ...
    def addStaff(self):
        ...
        if sid == '' or ssex == '' or sname == '' or spwd == '' \
                or srole == '' or sidcard == '' or sphone == '':
            QMessageBox().information(None, "提示", "信息不能为空！", QMessageBox.Yes)
            return False
        if int(self.staff.srole) > 1:
            ret = self.staff.addStaff(sid, sname, ssex, spwd, srole, sidcard, sphone)
            if ret:
